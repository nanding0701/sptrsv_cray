...
#pragma	omp	taskloop private (k,ii,lk) num_tasks(num_thread*8) nogroup
	...
	//	if ( !iam ) {
	//		printf("i1---iam=%d, L-solve time\t%f(%f), DGEM time\t %f, COMM time\t %f\n", iam, t,onesidecomm_rd, onesidedgemm[iam], onesidecomm_bc[iam]);
	//		fflush(stdout);
    //     } 
...
while( nfrecv1 < nfrecvx+nfrecvmod ){
        ...
        if ((nfrecvx>0) && (debug_count < nfrecvx)){
            ...
            //usleep(0); //err-10
                ...
                BC_subtotal[bcidx] = i;
                nfrecv1 += 1; //BC_subtotal[bcidx] - BCis_solved[bcidx];
			    BCis_solved[bcidx]++;
                        printf("sss-111--iam=%d.\n",iam);
                        fflush(stdout);
       ...
       for (rdidx=0;rdidx<Pc;rdidx++){
                ...
                nfrecv1 += 1; //RD_subtotal[rdidx]-RDis_solved[rdidx];
...
//PROFILE_BAROCLINIC_FINISH();
		...
		MPI_Reduce (&onesidecomm_rd, &tmax, 1, MPI_DOUBLE,
				MPI_MAX, 0, grid->comm);
		if ( !iam ) {
