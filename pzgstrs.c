...
#pragma	omp	taskloop private (k,ii,lk) num_tasks(num_thread*8) nogroup
...
#ifdef oneside
#if ( DEBUGlevel>=1 )
    printf("iam=%d, Leaf nodes solved--------\n",iam);
    fflush(stdout);
    //MPI_Barrier(MPI_COMM_WORLD);
    //for(int i=0; i<10*maxrecvsz*2; i++){
    //    printf("iam=%d,val[%d]=%lf\n",iam, i, BC_taskq[i]);
    //    fflush(stdout);
    //}
    //MPI_Barrier(MPI_COMM_WORLD);
#endif    
    while( nfrecv1 < nfrecvx+nfrecvmod ){
        ...
        if (totalsolveBC < nfrecvx){
            ...
            for (bcidx=0;bcidx<Pr && validBCQindex[bcidx]!=-1;bcidx++){
                ...
                k = (*recvbuf0).r;
#if ( DEBUGlevel>=1 )
                //printf("bcbc--222--iam=%d, k=%d, address(%d)=%lu\n",iam, k, bcidx, ibc);
                //fflush(stdout);
    ...
    if (totalsolveRD < nfrecvmod){
       ...
       for (rdidx=0;rdidx<Pc && validRDQindex[rdidx]!=-1;rdidx++){
                ...
                crc_16_val=crc_8((unsigned char*)&recvbuf0[LSUM_H],2*sizeof(double)*checkend);
#if ( DEBUGlevel>=1 )
